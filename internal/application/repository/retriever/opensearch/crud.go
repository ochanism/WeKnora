...
// of a 50MB allocation that's then rejected. Service layer (PR 3) is
...
// ErrBatchTooLarge; PR 3 introduces the async task path that lifts
// the cap.
//
...
// PR 3 can revisit via a custom raw request if telemetry shows the
// cost is material at production batch frequencies.
