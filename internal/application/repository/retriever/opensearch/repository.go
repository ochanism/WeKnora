...
//
// Concurrency: A single Repository instance is used by Phase 2 fan-out
// (retrieveFromStores) across N goroutines for the same store. The
// struct is therefore designed to be allocation-stable after
// construction; the only mutable state is the sync.Once + initErr maps,
...
// keywordsIndex returns the name of the dim-less keyword-only index
// used by the no-embedding save path (Phase 2 service layer's
// concurrentBatchSaveNoEmbedding). This index has the same mapping as
// the per-dim index minus the knn_vector field.
