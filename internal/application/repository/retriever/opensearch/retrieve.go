...
// the params.RetrieverType. Returns exactly one *RetrieveResult —
// Phase 2 fan-out (retrieveFromStores) depends on this 1-bundle-per-
...
// defensive default — Phase 2 fan-out always sets TopK. If you see
// TopK=10 in production logs, the caller is buggy. Caps at 10000 to
// stay below OpenSearch's index.max_result_window default.
...
// shape. Always returns exactly one *RetrieveResult — Phase 2's
// retrieveFromStores depends on this "one engine result per driver"
// contract.
//
// Defensive: warns if hit._id != _source.chunk_id (D12 invariant
// violation) but does not fail the query — the result is still usable
// for ranking, just suspicious.
