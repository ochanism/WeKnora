...
// the OpenSearch driver. Called by PR 3's container.go (env path) and
// engine_factory.go (DB-store path) — NOT by Repository.NewRepository,
// which receives the pre-built client. PR 2's dead-code invariant is
// preserved: no PR 2 code path activates this function; PR 3 wiring
// activates it.
//
// TLS posture (Sec-H3 + D-pr2-14):
...
//     (PR 1-added field). Default false. Operators set true ONLY for
...
//
// Transport tuning (D-pr2-15):
//   - MaxIdleConnsPerHost: 32   (Go default 2 is too low for Phase 2 fan-out)
