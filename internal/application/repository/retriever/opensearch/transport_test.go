...
// The Go default MaxIdleConnsPerHost is 2, which is too low for the
// Phase 2 fan-out goroutine count (defaultMultiStoreFanoutLimit).
func TestBuildHTTPTransport_PoolTuning(t *testing.T) {
	t.Parallel()
	tr := buildHTTPTransport(false)
	if tr.MaxIdleConnsPerHost < 32 {
		t.Errorf("MaxIdleConnsPerHost: want >=32 for Phase 2 fan-out, got %d",
